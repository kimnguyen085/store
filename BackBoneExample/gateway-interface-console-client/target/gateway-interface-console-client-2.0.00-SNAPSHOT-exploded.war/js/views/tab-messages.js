 ...
 * Tab Messages
 **/
window.TabMessages = TabContentView.extend({
	...
	prepareContent : function() {
		...
		this.messagesFilter = new GenericElementView({
			...
			templateName : 'partial/tab-messages-filter',
	...
	getDropdownContainer : function(source) {
		...
		var container = this.messagesFilter.$el.find(String.format('#message-participant-{0}-{1}-container', parts[0], parts[1]));
	...
	populateSortAndFilters : function() {
		...
		var directionItems = [];
		var actionItems = [];
		_(messageFilters.direction).each(function(obj, key) {
			directionItems.push({
				label : key,
				value : obj.value
			});
		});
		_(messageFilters.action).each(function(obj, key) {
			actionItems.push({
				label : key,
				value : obj.value
			});
		...
		this.messagesFilter.$el.find('button').on('click', function(e) {
			...
			if (type == 'filter') {
				...
				var sourceFundFilter = self.messagesFilter.$el.find('select[name="sourceABN"]').val();
				var sourceProductFilter = self.messagesFilter.$el.find('select[name="sourceUSI"]').val();
    ...
    clearData: function(){
        ...
        $("input[name=receivedDatetime_startValue]").val(dateFormat(today, app_config.datePattern)).trigger("change");
        $("input[name=receivedDatetime_endValue]").val(dateFormat(today, app_config.datePattern)).trigger("change");

        $("#tab-messages-type-filter").val('All').selectmenu('refresh');
        $("#tab-messages-direction-filter").val('All').selectmenu('refresh');
        $("#Messages-source-ABN").val('All').selectmenu('refresh');
        $("#Messages-source-USI").val('All').selectmenu('refresh');
        $("#Messages-target-ABN").val('All').selectmenu('refresh');
        $("#Messages-target-USI").val('All').selectmenu('refresh');
        $("#check-error-only").prop('checked', false).checkboxradio('refresh');
    },
		...
		this.tableView = new TableView({
			...
			bindings : {
				headers : [{
					...
					title : $.i18n.prop('message.info.label.source'),
					dataAttribute : 'sourceAbn,sourceUsi,sourceParticipantId.name,sourceParticipantId.productName',
					dataAttributeFormat : "<span class='tooltip-enabled source-fund' title='ABN: {0} - USI: {1}'><b>Name:</b> {2}<br/><b>Product:</b> {3}</span>",
					width : '19%'
				}, {
					title : $.i18n.prop('message.info.label.target'),
					dataAttribute : 'targetAbn,targetUsi,targetParticipantId.name,targetParticipantId.productName',
					dataAttributeFormat : "<span class='tooltip-enabled source-fund' title='ABN: {0} - USI: {1}'><b>Name:</b> {2}<br/><b>Product:</b> {3}</span>",
					width : '19%'
				}],
	...
	allReadyHandler : function() {
		this.detailsDropdowns.fund.source.setSelectedValue(this.filters.sourceABN);
		this.detailsDropdowns.product.source.setSelectedValue(this.filters.sourceUSI);
