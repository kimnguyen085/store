...
window.MenuCollapsible = Backbone.View.extend({
	...
	render : function() {
		...
		var $container = this.$el.find('.ui-collapsible-content');
		if ($container.length == 0) {
			this.$el.append('<div class=\"ui-collapsible-content\"></div>');
		};
		this.$el.find('.ui-collapsible-content').append(this.listView.render().el);
		return this;
	},
...
window.BindableListView = BindableListBaseView.extend({
	...
	initialize : function() {
		...
		this.subMenuSelect = this.options.subMenuSelect;
	},

	render : function() {
		BindableListBaseView.prototype.render.apply(this, arguments);
		if ( typeof this.firstSelect == 'number') {
			this.setSelectedIndex(this.firstSelect, this.subMenuSelect);
		}
		return this;
	},
	...
	setSelectedIndex : function(index, subIndex) {
		var childCount = this.itemViews.length || this.$el.children().length;
		if (this.selectable && index >= 0 && index < childCount) {
			if (this.itemView) {
				...
				if (subIndex !== undefined) {
					this.$el.children().eq(index).find('ul[data-role=listview]').children().eq(subIndex).addClass('selected');
				} else {
