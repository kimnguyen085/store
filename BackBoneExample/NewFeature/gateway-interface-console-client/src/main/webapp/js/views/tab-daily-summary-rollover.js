 ...
 * Tab Daily Summary
 **/
var dailySummaryRolloverFilters = {
	action : {
		"All" : {
			value : ""
		},
...
window.TabDailySummaryRollover = TabContentView.extend({
	...
	prepareFiltersAndContainers : function() {
        ...
        $('#rollover.filter-dropdown-table').hide(); // hide the advanced options table at the start

        // set the behavior of the Advanced Search toggle link
        this.$el.on('click','a[name=advancedSearchLinkRM]',function(e){
            $('#rollover.filter-dropdown-table').toggle("fast");
            $('a[name=advancedSearchLinkRM]').hide();
            $('a[name=hideAdvancedSearchLinkRM]').show();
        });

        // set the behavior of the Hide Advanced Search toggle link
        this.$el.on('click','a[name=hideAdvancedSearchLinkRM]',function(e){
            $('#rollover.filter-dropdown-table').toggle("fast");
            $('a[name=advancedSearchLinkRM]').show();
            $('a[name=hideAdvancedSearchLinkRM]').hide();
        });
    ...
    clearData: function(){
        ...
        this.$el.find("#Summary-source-ABN").val('All').selectmenu('refresh');
        this.$el.find("#Summary-source-USI").val('All').selectmenu('refresh');
	...
	prepareFilters : function() {
		...
		this.dailySummaryFilter = new GenericElementView({
			...
			templateName : 'partial/tab-summary-filter-rollover',
			messageTypeLabel : $.i18n.prop('message.type.rollover'),
	...
	populateSortAndFilters : function() {
		var self = this;
		var actionItems = [];
		_(dailySummaryRolloverFilters.action).each(function(obj, key) {
			actionItems.push({
				label : key,
				value : obj.value
			});
