window.ContributionMessagesPage = BasePage.extend({
	...
	initialize : function() {
		...
		this.pageId = "contribution messages";
        this.prepareData();
	},
	...
	createTabs : function() {
		 ...
		 * Summary Tab
		 **/
		this.tabDailySummary = new TabDailySummaryContribution({
			//model : this.model,
			messagePage : this
		});
		/**
		 * Message Tab
		 **/
		this.tabMessages = new TabContributionMessages({
			//model : this.model,
			messagePage : this
		});
        /**
	...
	drawFundOrProductDropdown : function(tab, source) {
		...
		if (parts[0] == "source" && parts[1] == "entity") {
			tab.detailsDropdowns[parts[0]][parts[1]] = new BindableDropdowView({
				...
				attributes : {
					"id" : String.format("{0}-{1}-{2}", tab.tabName, parts[0], parts[1]),
					"name" : String.format("{0}{1}", parts[0], parts[1]),
	...
	prepareDropdown : function(tab, source) {
		...
		if (parts[0] == 'fund') {
			this.attachDropdownEvent(tab, parts[1]);//not refractor
		}
	},
	...
	loadFundAndProductFilters : function(tab) {
		...
		tab.getDropdownContainer('source.entity').empty().addClass('data-loading');
		tab.participant.source.entity.fetch({
			data : data,
			success : function() {
				self.prepareDropdown(tab, 'source.entity');
			},
			error : function() {
				tab.setDropdownError('source.entity');
			}
		});
