 ...
 * Tab Daily Summary
 **/
var dailySummaryContributionFilters = {
	action : {
...
window.TabDailySummaryContribution = TabContentView.extend({
	...
	prepareFiltersAndContainers : function() {
        ...
        $('#contribution.filter-dropdown-table').hide(); // hide the advanced options table at the start

        // set the behavior of the Advanced Search toggle link
        this.$el.on('click','a[name=advancedSearchLinkCM]',function(e){
            $('#contribution.filter-dropdown-table').toggle("fast");
            $('a[name=advancedSearchLinkCM]').hide();
            $('a[name=hideAdvancedSearchLinkCM]').show();
        });

        // set the behavior of the Hide Advanced Search toggle link
        this.$el.on('click','a[name=hideAdvancedSearchLinkCM]',function(e){
            $('#contribution.filter-dropdown-table').toggle("fast");
            $('a[name=advancedSearchLinkCM]').show();
            $('a[name=hideAdvancedSearchLinkCM]').hide();
        });
    ...
    clearData: function(){
        ...
        this.$el.find("input[name=date_startValue]").val(dateFormat(today, app_config.datePattern)).trigger("change");
        this.$el.find("input[name=date_endValue]").val(dateFormat(today, app_config.datePattern)).trigger("change");

        this.$el.find("#tab-daily-summary-type-filter").val('All').selectmenu('refresh');
        this.$el.find("#Summary-source-entity").val('All').selectmenu('refresh');
        this.$el.find("#Summary-target-ABN").val('All').selectmenu('refresh');
        this.$el.find("#Summary-target-USI").val('All').selectmenu('refresh');
    },
	...
	loadData : function(clearFilters) {
        ...
        // make sure message type always has value
        if (data['action'] == undefined){
        	data['action'] = this.dailySummaryFilter.$el.find('select[name="action"]').val();
        }
	...
	prepareFilters : function() {
		...
		this.dailySummaryFilter = new GenericElementView({
			...
			templateName : 'partial/tab-summary-filter-contribution',
	...
	populateSortAndFilters : function() {
		var self = this;
		var actionItems = [];
		_(dailySummaryContributionFilters.action).each(function(obj, key) {
			actionItems.push({
				label : key,
				value : obj.value
			});
		...
		this.dailySummaryFilter.$el.find('button').on('click', function(e) {
			...
			if (type == 'filter') {
				...
				var sourceEntityFilter = self.dailySummaryFilter.$el.find('select[name="sourceentity"]').val();
	...
	getDropdownContainer : function(source) {
		...
		if (parts[0] == "source" && parts[1] == "entity") {
			container = this.dailySummaryFilter.$el.find(String.format('#daily-summary-{0}-{1}-container', parts[0], parts[1]));
		}else {
			container = this.dailySummaryFilter.$el.find(String.format('#daily-summary-participant-{0}-{1}-container', parts[0], parts[1]));
		}
	...
	showTabMessages : function(dailySummaryDate, dailySummaryDirection, dailySummaryType, dailySummaryError,summaryEbmsMessageId,summaryConversationId) {
		...
		var sourceEntityFilter = this.dailySummaryFilter.$el.find('select[name="sourceentity"]').val();
